...
def get_alpha(x, id):
    ...
    mean_x = sum(x) / len(x)
    if cur_x > mean_x:
        return (cur_x - mean_x) / (max_x - mean_x)
    else:
        return (cur_x - mean_x) / (min_x - mean_x)
...
pweight = 1
model_dir = 'outputs/block/gridworld-vec-fix-reinforce-wprior-' + str(pweight) + 'decay0.99'
...
while episode < EPISODES:
    ...
    while not done:
        ...
        if done or step == STEPS:
            ...
            tb_writer.add_scalar("prior weight", pweight, episode)
    ...
    r = (1 - pweight) * np.array(r) + pweight * np.array(shadow_r)
    # r = np.array(shadow_r)
    r = r.tolist()
    ...
    for t in range(len(r)):
        ...
        tri = plt.Polygon(triloc, color=get_color(shadow_r, t), alpha=get_alpha(shadow_r, t))
        axs[0, 1].add_patch(tri)
        # total return
        axs[1, 0].text(textloc[0], textloc[1], round(discounted_rewards[t], 2))
        tri = plt.Polygon(triloc, color=get_color(discounted_rewards, t), alpha=get_alpha(discounted_rewards, t))
        axs[1, 0].add_patch(tri)
        # shadow return
        axs[1, 1].text(textloc[0], textloc[1], round(discounted_shadowrs[t], 2))
        tri = plt.Polygon(triloc, color=get_color(discounted_shadowrs, t), alpha=get_alpha(discounted_shadowrs, t))
