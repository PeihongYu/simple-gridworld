...
for i in range(len(env.agent_pos)):
    models.append(ACModel(4, 4).to(DEVICE))
...
while episode < EPISODES:
    ...
    while not done:
        ...
        action_2a = []
        for i in range(2):
            actions, value = models[i](state["vec"])
        ...
        if done:
            ...
            state = env.reset()
            for i in range(2):
                all_rewards[i].append(np.sum(r[i]))
            all_eplens.append(step)
            step = 0
            print(f"EPISODE {episode} STEP: {pd.Series(all_eplens).tail(100).mean():.2f} SCORE: {np.sum(r[0]):.2f}, {np.sum(r[1]):.2f} "
